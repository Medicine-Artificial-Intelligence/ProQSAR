...
class StatisticalAnalysis:
    ...
    def extract_scoring_dfs(report_df: pd.DataFrame, scoring: list | str):
        ...
        df_long.rename(columns={"index": "Fold"}, inplace=True)

        return df_long

    @staticmethod
    def check_variance_homogeneity(
        report_df: pd.DataFrame, scoring: list | str, levene_test: bool = True
    ):
        ...
        for metric in scoring:
            ...
            if levene_test:
                groups = [
                    group["Value"].values for _, group in metric_data.groupby("Method")
                ]
                stat, p_value = levene(*groups)
                metric_result["p_value"] = p_value
    ...
    def check_normality(report_df: pd.DataFrame, scoring: list | str):
        ...
        for i, metric in enumerate(scorings):
            ax = axes[0, i]
            sns.histplot(
                df_norm[df_norm["Scoring"] == metric]["Value"], kde=True, ax=ax
            )
    ...
    def make_ANOVARM(report_df, scoring: list | str):
        
        if isinstance(scoring, str):
            scoring = [scoring]

        for i, stat in enumerate(scoring):
            model = AnovaRM(data=report_df, depvar=stat, subject='cv_cycle', within=['method']).fit()
            p_value = model.anova_table['Pr > F'].iloc[0]
            
        #### co nen code visualize? neu co thi xai lai code cua model_validation hay code rieng moi?
