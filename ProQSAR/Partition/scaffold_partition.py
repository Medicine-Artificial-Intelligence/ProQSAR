...
class ScaffoldPartition:
    ...
    def scaffold(self, data: pd.DataFrame, smiles_col: str) -> List[List[int]]:
        """
    ...
    def fit(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        """
        scaffold_lists = self.scaffold(self.data, self.smiles_col)
        count_list = [len(i) for i in scaffold_lists]
        if np.array(count_list).sum() != len(self.data):
