...
class TestStatisticalAnalysis(unittest.TestCase):
    ...
    def setUp(self):
        self.cv_class = pd.read_csv("Data/cv_class.csv").set_index(['scoring', 'cv_cycle'])
    ...
    def test_extract_scoring_dfs(self):
        scoring_dfs, scoring_list, method_list = StatisticalAnalysis.extract_scoring_dfs(
            self.cv_class, 
            scoring_list='accuracy',
            method_list=["KNeighborsClassifier", "SVC", "ExtraTreesClassifier"])
        self.assertEqual(scoring_list, ['accuracy'])
        self.assertListEqual(method_list, ["KNeighborsClassifier", "SVC", "ExtraTreesClassifier"])
    ...
    def test_check_variance_homogeneity(self):
        ...
        self.assertTrue(os.path.exists(f"{self.temp_dir.name}/check_variance_homogeneity.csv"))
    ...
    def test_check_normality(self):
        ...
        self.assertTrue(
            os.path.exists(f"{self.temp_dir.name}/check_normality.png")
        )
        

    def test_anova_test(self):
        with self.assertRaises(ValueError):
            StatisticalAnalysis.test(self.cv_reg, scoring_list='r2', select_test='invalid_test')
        StatisticalAnalysis.test(
            ...
            fig_name='AnovaRM')
        self.assertTrue(
            os.path.exists(f"{self.temp_dir.name}/AnovaRM.png")
        )
    ...
    def test_friedman_test(self):
        StatisticalAnalysis.test(
            ...
            fig_name='friedman')
        self.assertTrue(
            os.path.exists(f"{self.temp_dir.name}/friedman.png")
        )
    ...
    def test_posthoc_conover_friedman(self):
        ...
        self.assertTrue(
            os.path.exists(f"{self.temp_dir.name}/cofried_sign_plot.png"))
        self.assertTrue(
            os.path.exists(f"{self.temp_dir.name}/cofried_ccd.png"))
        self.assertTrue(
            os.path.exists(f"{self.temp_dir.name}/cofried_pc_r2.csv"))
    ...
    def test_posthoc_tukeyhsd(self):
        ...
        self.assertTrue(
            os.path.exists(f"{self.temp_dir.name}/tukey_result_tab_f1.csv"))
        self.assertTrue(
            os.path.exists(f"{self.temp_dir.name}/tukey_result_tab_accuracy.csv"))
        self.assertTrue(
            os.path.exists(f"{self.temp_dir.name}/tukey_mcs.png"))
        self.assertTrue(
            os.path.exists(f"{self.temp_dir.name}/tukey_ci.png"))
