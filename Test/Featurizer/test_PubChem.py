...
from ProQSAR.Featurizer.PubChem import (
...
class TestPubChemFingerprints(unittest.TestCase):
	...
	def test_part1_is_nonempty_and_distinguishes_simple_molecules(self):
		...
		self.assertEqual(tuple(bv_cno.GetOnBits()), tuple(calcPubChemFingerPart1(self.methanolamine).GetOnBits()))
		self.assertEqual(tuple(bv_ccc.GetOnBits()), tuple(calcPubChemFingerPart1(self.propane).GetOnBits()))
	...
	def test_funcs_2_to_7_bond_ring_branches(self):
		...
		bonds = {
			0: StubBond("SINGLE", 6, 6), 1: StubBond("SINGLE", 6, 6), 2: StubBond("SINGLE", 6, 6),
			3: StubBond("AROMATIC", 6, 6), 4: StubBond("AROMATIC", 6, 6), 5: StubBond("AROMATIC", 6, 6),
			6: StubBond("AROMATIC", 7, 6), 7: StubBond("AROMATIC", 6, 6), 8: StubBond("AROMATIC", 6, 6),
			9: StubBond("AROMATIC", 8, 6), 10: StubBond("SINGLE", 7, 6), 11: StubBond("SINGLE", 8, 6),
		}
		...
		rings = [
			...
			(10,1,2,0),         # unsaturated non-aromatic N-containing (due to non-single elsewhere)
